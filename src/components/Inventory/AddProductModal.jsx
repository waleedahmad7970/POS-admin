...
export default function AddProductModal({ onClose, onProductAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Failed to add product. Make sure the name is unique.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center space-x-3 pt-2">
            ...
            <label htmlFor="isWeighed" className="text-sm font-medium text-text cursor-pointer">
              Item is weighed on scale (e.g. Produce)
            </label>
          </div>

          {!formData.isWeighed && (
            <div>
              <label className="block text-sm font-semibold text-secondary mb-1">Initial Stock</label>
              <input 
                type="number" 
                min="0"
                required
          ...
          <div className="pt-4 flex justify-end space-x-3">
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={18} /> : <span>Add Product</span>}
            </button>
          </div>
